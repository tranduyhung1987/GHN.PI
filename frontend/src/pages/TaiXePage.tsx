...
export default function TaiXePage() {
  ...
  const { filter, setFilter, orders, selectedOrder, setSelectedOrder } = useTaiXe();
  ...
  return (
    <div style={pageContainer}>
      ...
      <div style={tabContainer}>
        <button style={filter === 'pending' ? activeTabStyle : inactiveTabStyle} onClick={() => setFilter('pending')}>Chờ lấy</button>
        <button style={filter === 'shipping' ? activeTabStyle : inactiveTabStyle} onClick={() => setFilter('shipping')}>Đang giao</button>
        <button style={filter === 'completed' ? activeTabStyle : inactiveTabStyle} onClick={() => setFilter('completed')}>Hoàn thành</button>
        <button style={filter === 'all' ? activeTabStyle : inactiveTabStyle} onClick={() => setFilter('all')}>Tất cả</button>
      </div>
            ...
            <div key={order.maDon} style={orderCardStyle} onClick={() => setSelectedOrder(order)}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <p style={{ fontSize: '13px', color: '#666' }}>{order.address}</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={statusStyle(order.status)}>{order.status.toUpperCase()}</span>
                  <p style={{ marginTop: '8px', fontWeight: '700', color: '#22d3ee' }}>{order.fee}</p>
                </div>
              </div>
            </div>
          ))
      ...
      {selectedOrder && (
        <div style={cardStyle}>
          <h4 style={sectionTitle}>Chi tiết: {selectedOrder.maDon}</h4>
          <p><strong>Khách:</strong> {selectedOrder.customer}</p>
          <p><strong>Địa chỉ:</strong> {selectedOrder.address}</p>
          <p><strong>Phí ship:</strong> {selectedOrder.fee}</p>
          <p><strong>Loại:</strong> {selectedOrder.loai}</p>
          {selectedOrder.ghiChu && <p><strong>Ghi chú:</strong> {selectedOrder.ghiChu}</p>}
          <button style={submitButton} onClick={() => alert(`Đã nhận đơn ${selectedOrder.maDon}`)}>
...
const orderCardStyle: React.CSSProperties = { background: '#f8f7ff', padding: '16px', borderRadius: '12px', marginBottom: '12px', border: '1px solid #e0e7ff', cursor: 'pointer' };
const submitButton: React.CSSProperties = { padding: '16px', background: '#4c1d95', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '700', width: '100%', marginTop: '10px' };

const statusStyle = (status: string): React.CSSProperties => {
  let color = '#eab308';
  if (status === 'shipping') color = '#22d3ee';
  if (status === 'completed') color = '#22c55e';
  return { padding: '4px 10px', borderRadius: '99px', fontSize: '12px', fontWeight: '700', background: '#f1f5f9', color };
};